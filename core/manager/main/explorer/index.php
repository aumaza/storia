...
$root_path = '../../../sqls/';
// or Input the path what you choose. example: ../path/to/your/directory/
...
if (!@is_dir($root_path)) {
    echo sprintf('<h1>Directorio Raíz "%s" no encontrado!</h1>', fm_enc($root_path));
    exit;
}
...
if ($use_auth) {
    if (isset($_SESSION['logged'], $auth_users[$_SESSION['logged']])) {
        ...
        if (isset($auth_users[$_POST['fm_usr']]) && $_POST['fm_pwd'] === $auth_users[$_POST['fm_usr']]) {
            ...
            fm_set_msg('Bienvenido/a a Explorer ' .$_POST['fm_usr']);
        ...
        <div class="path">
            <form action="" method="post" style="margin:10px;text-align:center">
                <input name="fm_usr" value="" placeholder="Usuario" required>
...
if (isset($_GET['del'])) {
    ...
    if ($del != '' && $del != '..' && $del != '.') {
        ...
        if (fm_rdelete($path . '/' . $del)) {
            $msg = $is_dir ? 'Directorio <b>%s</b> borrado' : 'Archivo <b>%s</b> borrado';
            fm_set_msg(sprintf($msg, fm_enc($del)));
        } else {
            $msg = $is_dir ? 'Directorio <b>%s</b> no borrado' : 'Archivo <b>%s</b> no borrado';
            fm_set_msg(sprintf($msg, fm_enc($del)), 'error');
        }
    } else {
        fm_set_msg('Nombre de Archivo o Directorio equivocado', 'error');
    }
...
if (isset($_GET['new'])) {
    ...
    if ($new != '' && $new != '..' && $new != '.') {
        ...
        if (fm_mkdir($path . '/' . $new, false) === true) {
            fm_set_msg(sprintf('Directorio <b>%s</b> creado', fm_enc($new)));
        } elseif (fm_mkdir($path . '/' . $new, false) === $path . '/' . $new) {
            fm_set_msg(sprintf('Directorio <b>%s</b> existente', fm_enc($new)), 'alert');
        } else {
            fm_set_msg(sprintf('Directorio <b>%s</b> no creado', fm_enc($new)), 'error');
        }
    } else {
        fm_set_msg('Nombre de Directorio Erroneo', 'error');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
    ...
    if ($from != $dest) {
        ...
        if ($move) {
            ...
            if ($rename) {
                ...
                fm_set_msg('Archivo o Directorio Existente en esta ubicación', 'alert');
            } else {
...
if (isset($_POST['file'], $_POST['copy_to'], $_POST['finish'])) {
    ...
    if (!is_dir($copy_to_path)) {
        if (!fm_mkdir($copy_to_path, true)) {
            fm_set_msg('Imposible crear Directorio de destino', 'error');
            fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
        }
    }
    // move?
    ...
    if (is_array($files) && count($files)) {
        ...
        if ($errors == 0) {
            $msg = $move ? 'Seleccione los Archivos o Directorios a Mover' : 'Seleccione Archivos o Directorios a copiar';
            fm_set_msg($msg);
        } else {
...
if (isset($_GET['ren'], $_GET['to'])) {
    ...
    if ($old != '' && $new != '') {
        if (fm_rename($path . '/' . $old, $path . '/' . $new)) {
            ...
            fm_set_msg(sprintf('Error mientras se renombraba desde <b>%s</b> a <b>%s</b>', fm_enc($old), fm_enc($new)), 'error');
        }
    } else {
...
if (isset($_POST['group'], $_POST['delete'])) {
    ...
    if (is_array($files) && count($files)) {
        ...
        if ($errors == 0) {
            fm_set_msg('Seleccione Archivos o Directorios a borrar');
        } else {
...
if (isset($_GET['upload'])) {
    ...
    <div class="path">
        ...
        <p class="break-word">Directorio de Destino: <?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . FM_PATH)) ?></p>
    ...
    <div class="path">
        ...
        <form action="" method="post">
            ...
            <p class="break-word">Source folder: <?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . FM_PATH)) ?><br>
                <label for="inp_copy_to">Directorio de Destino:</label>
...
if (isset($_GET['copy']) && !isset($_GET['finish'])) {
    ...
    <div class="path">
        <p><b>Copiando</b></p>
        <p class="break-word">
            Ruta Directorio: <?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . $copy)) ?><br>
            Directorio de Destino: <?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . FM_PATH)) ?>
        </p>
        <p>
    ...
    <div class="path">
        ...
        <p class="break-word">
            ...
            if ($is_zip && $filenames !== false) {
                ...
                Compresion: <?php echo round(($total_comp / $total_uncomp) * 100) ?>%<br>
                <?php
            }
            // Image info
            if ($is_image) {
...
<?php endif; ?>
<td>
<a title="Borrar" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('Delete folder?');"><i class="icon-cross"></i></a>
...
<a title="Enlazar con..." href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f . '/') ?>" target="_blank"><i class="icon-chain"></i></a>
</td></tr>
    <?php
    flush();
}
...
<?php endif; ?>
<td>
<a title="Borrar" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('Delete file?');"><i class="icon-cross"></i></a>
<a title="Renombrar" href="#" onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc($f) ?>');return false;"><i class="icon-rename"></i></a>
...
if (empty($folders) && empty($files)) {
    ?>
<tr><td></td><td colspan="<?php echo !FM_IS_WIN ? '6' : '4' ?>"><em>Directorio Vacío</em></td></tr>
<?php
} else {
    ?>
...
<p><input type="submit" name="delete" value="Borrar" onclick="return confirm('Delete selected files and folders?')">
...
<?php if (FM_USE_AUTH): ?><a title="Salir" href="?logout=1"><i class="icon-logout"></i></a><?php endif; ?>
</div>
        <?php
        $path = fm_clean_path($path);
...
{
    ?>
<p class="center"><small>Bienvenido a Explorer de Storia</small></p><hr>
</div>
<script>
